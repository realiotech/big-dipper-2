...
export default function VotingPower({ data, status }) {
    ...
    const votingPowerPercent = status === 3 ? numeral((data.self / (numeral(data.overall.value).value() ?? 0)) * 100) : numeral(0)
    return (
        <Box
            bg="#F6F7F8"
            p={6}
        ...
        >
            ...
            <Text fontSize="2xl" fontWeight="bold" color="blue.500">
                ...
                <ProgressRoot variant="subtle" value={votingPowerPercent.value()}>
                    <ProgressBar />
                </ProgressRoot>
            </Text>
            <VStack align="flex-start">
                <Flex justify="space-between" w="100%">
                    <Text>Block</Text>
                    <HelpLink href={BLOCK_DETAILS(data.height)} value={numeral(data.height).format('0,0')} />
                </Flex>
                <Flex justify="space-between" w="100%">
